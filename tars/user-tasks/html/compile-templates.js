...
const { replace } = tars.packages;
const { plumber } = tars.packages;
const { rename } = tars.packages;
const { through2 } = tars.packages;
const fs = require('fs');

const { notifier } = tars.helpers;
const { browserSync } = tars.packages;
...
if (
  ...
  tars.config.svg.workflow === 'symbols' &&
    ...
    replacement: () => {
      ...
      try {
        return fs.readFileSync(
          `${tars.config.devPath}temp/svg-symbols${tars.options.build.hash}.svg`,
          'utf8'
        );
      } catch (error) {
        return '';
