...
module.exports = () => {
  return tars.packages.chokidar
    .watch(`markup/${tars.config.fs.componentsFolderName}/**/data/data.js`, tars.options.watch)
    .on('all', (event, watchedPath) => {
      tars.helpers.watcherLog(event, watchedPath);

      // Remove cache for components with changed data-files
      if (
        (templaterName === 'jade' || templaterName === 'pug') &&
        tars.packages.cache.caches &&
      ...
      runSequence(
        ['html:concat-mocks-data', 'html:compile-templates'],
        ['html:process-bem-naming'],
        () => {}
      );
    });
};
